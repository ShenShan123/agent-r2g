...
- **Latent (documented, not changed):** `mk_sky130_project.py` reads `ppa.get("cell_count")` but `extract_ppa.py` writes the count under `geometry.instance_count`, so `cell_count` is always 0 and `use_floor` is always true (every design takes the explicit-DIE branch). This is harmless given the PDN floor + the new pin floor, and *deliberately* left as-is: making the read live would flip 100+ validated small designs from the 200 um floor to utilization-sizing (timing/PDN re-validation risk) for no current benefit. Revisit only if a genuinely large-core design (≫5000 cells) enters the corpus and over-packs the 200 um floor.
