...
def _insert_trajectory(conn, *, fix_session_id, design_name, design_family,
    ...
    conn.execute(
        ...
        (fix_session_id, f"/tmp/{fix_session_id}", design_name, design_family,
         platform, check_type, violation_class, "[]", 1, outcome,
...
def test_mine_emits_fix_candidates(tmp_knowledge_dir):
    ...
    # 1 abandoned episode in the same group (drives clearance_rate below 1.0)
    _insert_trajectory(
        conn, fix_session_id="sess_aband", design_name="eth_loop",
        design_family="verilog_ethernet", platform="nangate45",
        check_type="drc", violation_class="M3_ANTENNA",
        outcome="abandoned", winning_strategy="antenna_diode_repair",
    )
    ...
    # Only the verilog_ethernet/drc/M3_ANTENNA group clears resolved>=3
    assert len(cands) == 1
    c = cands[0]
    ...
    assert abs(c["clearance_rate"] - (4 / 5)) < 1e-9
