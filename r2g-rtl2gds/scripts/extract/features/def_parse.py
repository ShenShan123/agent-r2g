"""Shared DEF / SDC parsing helpers for the feature-extraction workers.

These were duplicated verbatim across the original feature_test_v2 workers; they are
consolidated here unchanged in behavior. The component and net parsers return the
richest superset of fields any single worker needs (status/orient for components,
routes/use for nets) — callers read only the fields they care about, so the byte-level
output of every worker is preserved (validated by tests/test_feature_regression.py).

Hand-rolled line parsers (no DEF grammar library) keyed to ORFS ``write_def`` output.
Pure stdlib.
"""
import os
import re
